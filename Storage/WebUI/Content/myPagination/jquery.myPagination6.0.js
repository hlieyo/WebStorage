    ...
    function init(param, obj) {
        ...
        function getPanelTipInfo() {
            ...
            if (opts.panel.tipInfo_on) {
                ...
                if (-1 != c.indexOf("{input}")) {
                    ...
                    c = c.replace("{sumPageRow}", opts.pageRowCount),
                    b.html(c),
        ...
        function responseHandle(data) {
            var pageCountId = opts.ajax.pageCountId,
                pageRowCountId = opts.ajax.pageRowCountId,
                resultPageCount = 1,
            ...
            switch (opts.ajax.dataType) {
                case "json":
                    try {
                        ...
                        resultpageRowCount = eval("data." + pageRowCountId);
                    }
                    break;
                case "xml":
                    try {
                        resultPageCount = $(data).find(pageCountId).text();
                        resultpageRowCount = $(data).find(pageRowCountId).text();
                    } catch (e) {
            ...
            opts.pageRowCount = resultpageRowCount,
